...
export function CourseRatingsTable() {
  ...
  const columns = [
    ...
    {
      accessorKey: "batches[0]",
      header: "Batch",
      cell: ({ row }) => (
        <p className="w-[200px]">
          Batch From: {row.original?.batches[0]?.date},{" "}
          {row.original?.batches[0]?.time} IST
        </p>
      ),
      filterFn: (row, columnId, filterValue) => {
        const batchInfo = `Batch From: ${row.original?.batches[0]?.date}, ${row.original?.batches[0]?.time} IST`;
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Dialog>
          ...
          <DialogContent>
            <div className="p-4">{row.getValue("feedback")}</div>
          </DialogContent>
        </Dialog>
      ),
    },
  ...
  const handleDelete = async (row) => {
    try {
      await deleteDoc(doc(db, "course-ratings", row.original.docId));
        ...
        <div className="flex gap-4 items-center justify-between flex-wrap w-full">
          <div className="flex gap-4 items-center flex-wrap max-w-[800px]">
            ...
            <Input
              ...
              value={table.getColumn("batches[0]")?.getFilterValue() ?? ""}
              onChange={(event) =>
                table
                  .getColumn("batches[0]")
                  ?.setFilterValue(event.target.value)
